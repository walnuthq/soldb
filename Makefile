...
wasm-check:
	...
	cargo clippy --target $(WASM_TARGET) -p soldb-rpc -p soldb-wasm --no-default-features -- -D warnings
	cargo clippy -p soldb-rpc -p soldb-wasm --no-default-features --all-targets -- -D warnings
	cargo test -p soldb-rpc -p soldb-wasm --no-default-features
