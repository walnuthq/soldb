            ...
            if step.op in ["CALL", "DELEGATECALL", "STATICCALL"]:
                ...
                if len(step.stack) >= 6:
                    ...
                    if len(step.stack) >= required_stack_size:
                        ...
                        target_name = "Unknown"
                        if self.multi_contract_parser:
                            target_contract = self.multi_contract_parser.get_contract_at_address(to_addr)
                            if target_contract:
                        ...
                        if calldata and len(calldata) >= 10:
                            selector = calldata[:10]
                            if hasattr(self, 'function_signatures'):
                                func_info = self.function_signatures.get(selector)
                                if func_info:
