...
normalized compiler debug programs. It indexes each program once, attributes
dynamic `gas_cost` samples by bytecode address and creation/runtime environment,
and returns sorted contract, function, source-line, opcode, and instruction
tables. It also returns gas-weighted folded stacks for flame graph renderers.
